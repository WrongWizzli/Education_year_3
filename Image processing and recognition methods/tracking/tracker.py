...
class Tracker:
    ...
    def bind_tracklet(self, detections):
        ...
        for i in range(detections.shape[0]):
            for j in range(prev_detections.shape[0]):
                IoUs.append([iou_score(detections[i][1:], prev_detections[j][1:]), i, j])
        # Step 2: sort IOU list
        IoUs = sorted(IoUs, key=lambda x: x[0], reverse=True)
        # Step 3: fill detections[:, 0] with best match
        maski = np.zeros((len(detections),), dtype='uint8')
        maskj = np.zeros((len(prev_detections),), dtype='uint8')
        for IoU in IoUs:
            if not maski[IoU[1]] and not maskj[IoU[2]]:
                maski[IoU[1]] = 1
                maskj[IoU[2]] = 1
                detections[IoU[1]][0] = prev_detections[IoU[2]][0]
        # One matching for each id
        for i in range(maski.shape[0]):
            if not maski[i]:
                detections[i][0] = self.new_label()
